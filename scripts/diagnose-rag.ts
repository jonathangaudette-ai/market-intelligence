...
async function main() {
  try {
    ...
    recentDocs.forEach((doc, idx) => {
      ...
      console.log(`    rfpOutcome: ${metadata?.rfpOutcome || 'N/A'}`);
    ...
    const docsWithText = recentDocs.filter(doc => (doc.metadata as any)?.extractedText);
    ...
    const filterTests: Array<{ name: string; filter: any }> = [
      {
    ...
    for (const test of filterTests) {
      ...
      if (queryResponse.matches.length > 0) {
        queryResponse.matches.forEach((match, idx) => {
          ...
          console.log(`    Extrait: "${(match.metadata?.text as string || '').slice(0, 150)}..."`);
        });
      } else {
