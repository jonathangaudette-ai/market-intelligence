/**
 * CleanItSupply Scraper - Scraper for cleanitsupply.ca
 * CleanItSupply is a wholesale supplier with competitive pricing
 */
...
export class CleanItSupplyScraper extends BaseScraper {
  constructor(config: CompetitorConfig) {
    super(config);
  }

  /**
   * Search for a product by SKU on CleanItSupply
   */
  ...
  /**
   * Search for a product by name and brand on CleanItSupply
   */
  ...
  async extractProductDetails(url: string): Promise<ProductDetails> {
    ...
    try {
      ...
      if (skuEl) {
        ...
        // Extract SKU from text like "SKU: 12345" or just "12345"
        const skuMatch = skuText.match(/(?:SKU|Item|Product)[\s#:]+([A-Z0-9\-]+)/i);
      ...
      try {
        const availabilityEl = await this.page.$('.stock-status, .availability, .in-stock, .out-of-stock');
        if (availabilityEl) {
